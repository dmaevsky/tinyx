...
const select = ({ subscribe, update, get, commit }, selector) => {
  ...
  return {
    ...
    set: value => update(state => setIn(state, ...selector(state), value)),
    update: updater => update(state => updateIn(state, ...selector(state), updater)),
